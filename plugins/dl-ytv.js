import * as {youtubedl, youtubedlv2} from '@bochilteam/scraper';
...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  ...
  try {
    ...
    const ttl_1 = `${yt_1?.title ? yt_1.title : 'DOWNLOADED'}`;
    const fileSizeInBytes = buff.byteLength;
    const fileSizeInKB = fileSizeInBytes / 1024;
    const fileSizeInMB = fileSizeInKB / 1024;
    const roundedFileSizeInMB = fileSizeInMB.toFixed(2);
   if (fileSizeInMB > 200) {
    await conn.sendMessage(m.chat, {document: buff, caption: `*🎉${mssg.title}:* ${ttl_1}\n*${mssg.size}:* ${roundedFileSizeInMB} MB`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
    await conn.sendMessage(m.chat, {text: `*✅ Download completed.*\n\n*It was sent in document format due to the weight of the video ${roundedFileSizeInMB} MB and exceeds the limit set by WhatsApp .*\n*🎉${mssg.title}:* ${ttl_1}`, edit: key}, {quoted: m});
    enviando = false
   } else {
    await conn.sendMessage(m.chat, {video: buff, caption: `*🎉${mssg.title}:* ${ttl_1}\n*${mssg.size}:* ${roundedFileSizeInMB} MB`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
    await conn.sendMessage(m.chat, {text: `*✅ Download completed.*`, edit: key}, {quoted: m});
    enviando = false   
   }
 } catch (ee) {
    console.log(ee)
  try {
    ...
    await conn.sendMessage(m.chat, {video: {url: dl_url}, fileName: `${ttl}.mp4`, mimetype: 'video/mp4', caption: `*🎉${mssg.title}:* ${ttl}\n*⚖️${mssg.size}:* ${size}`, thumbnail: await fetch(yt.thumbnail)}, {quoted: m});
    await conn.sendMessage(m.chat, {text: '*✅ Download completed.*', edit: key}, {quoted: m});
    enviando = false
  } catch (ee2) {
    console.log(ee2)
    try {
      ...
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `⚡𝑷-𝑴𝑫⚡`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
      await conn.sendMessage(m.chat, {text: '*✅ Download completed.*', edit: key}, {quoted: m});
      enviando = false
    } catch {
      try {
        ...
        await conn.sendMessage(m.chat, {video: {url: n2}, fileName: `${n}.mp4`, mimetype: 'video/mp4', caption: `*🎉${mssg.title}:* ${n}\n*⚖️${mssg.size}:* ${n3}`, thumbnail: await fetch(n4)}, {quoted: m});
        await conn.sendMessage(m.chat, {text: '*✅ Download completed.*', edit: key}, {quoted: m});
        enviando = false
      } catch {
...
handler.command = /^(fgmp4|dlmp4|yt(v|mp4)?)$/i;
